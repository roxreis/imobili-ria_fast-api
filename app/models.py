...
class Transaction(Base):
    ...
    transaction_id = Column(UUID(as_uuid=True), primary_key=True, default=lambda: str(uuid.uuid4()))
...
class Party(Base):
    ...
    party_id = Column(UUID(as_uuid=True), primary_key=True, default=lambda: str(uuid.uuid4()))
    transaction_id_fk = Column(UUID(as_uuid=True), ForeignKey("transactions.transaction_id"), nullable=False)
...
class Commission(Base):
    ...
    commission_id = Column(UUID(as_uuid=True), primary_key=True, default=lambda: str(uuid.uuid4()))
    transaction_id_fk = Column(UUID(as_uuid=True), ForeignKey("transactions.transaction_id"), nullable=False)
